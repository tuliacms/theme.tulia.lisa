...
{% set address = customizer_get('lisa.footer.contact.address') %}
{% set phone = customizer_get('lisa.footer.contact.phone') %}
{% set email = customizer_get('lisa.footer.contact.email') %}
<div class="contact-row contact-address">{{ address|nl2br }}</div>
{% if phone %}
    <div class="contact-row contact-phone"><a href="tel:{{ phone }}"><i class="cr-icon fas fa-phone-alt"></i><span>{{ phone }}</span></a></div>
{% endif %}
{% if email %}
    <div class="contact-row contact-email"><a href="mailto:{{ email }}"><i class="cr-icon fas fa-envelope"></i><span>{{ email }}</span></a></div>
{% endif %}
